...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    val state = rememberEditableUserInputState(hint = "Choose Destination")
    ...
    val currentOnToDestinationChanged by rememberUpdatedState(
        newValue = onToDestinationChanged
    )

    LaunchedEffect(state){
        //Convert state to Kotlin Flow
        snapshotFlow { state.text }
            .filter { !state.isHint }
            .collect{
                currentOnToDestinationChanged(state.text)
            }
    }
}
